...
class SeriesAndEpisodeTests extends DatabaseTests {
  ...
  void testEpisodeCRUD() throws SQLException, NoResultException {
    try (Connection connection = databaseConnector.getConnection()) {
      ...
      series.save(connection);
      Series.Episode episode = series.new EpisodeBuilder(randomString(), randomString(), 100, (short) 1, (short) 1).build();
  ...
  void testEpisodeWithInvalidSeasonAndEpisodeValues() throws SQLException {
    try (Connection connection = databaseConnector.getConnection()) {
      ...
      Series series = new Series(String.format(randomString(), new Date().getTime()));
      series.save(connection);
      Series.Episode episode = series.new EpisodeBuilder(
          randomString(),
          randomString(),
          100,
          (short) 2,
  ...
  void testEpisodeWithAutomaticEpisodeNumber() throws SQLException {
    try (Connection connection = databaseConnector.getConnection()) {
      ...
      Series series = new Series(String.format(randomString(), new Date().getTime()));
      series.save(connection);
      Series.Episode episode1 = series.new EpisodeBuilder(randomString(), randomString(), 100, (short) 1).build();
      assertEquals(1, episode1.getEpisodeNumber());
      Series.Episode episode2 = series.new EpisodeBuilder(randomString(), randomString(), 100, (short) 1).build();
